...
export const SearchView: React.FC<SearchViewProps> = ({
  ...
  const handleLoadMore = async () => {
    if (!selectedGenre || loadingMore || !hasMore) return;
    setLoadingMore(true);
    try {
    ...
    <div className="fixed inset-0 z-50 bg-zinc-950/98 backdrop-blur-2xl flex flex-col p-4 animate-fade-in text-zinc-200">
      ...
      <div className="flex items-center gap-3 pt-2 pb-4 border-b border-zinc-800">
        <div className="flex-1 relative flex items-center">
          ...
          <input
            type="text"
            placeholder="Search anime title, genres..."
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="w-full bg-zinc-900 text-sm text-zinc-100 placeholder-zinc-500 pl-9 pr-8 py-2.5 rounded-2xl border border-zinc-800 focus:outline-none focus:border-zinc-600"
          />
          {query && (
            <button onClick={() => setQuery('')} className="absolute right-3 text-zinc-400 hover:text-zinc-200">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
      ...
      {/* Genre Filter Pills */}
      <div className="py-3 flex gap-2 overflow-x-auto no-scrollbar border-b border-zinc-850 shrink-0">
        <button
      ...
      <div className="py-2.5 px-1.5 border-b border-zinc-900 flex flex-wrap gap-x-4 gap-y-2.5 items-center text-xs text-zinc-400 select-none bg-zinc-950 shrink-0">
      ...
      <div className="flex-1 overflow-y-auto py-4">
        <div className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-3 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            ...
            {loading ? 'Searching live API...' : `${filteredResults.length} Anime Streams Found`}
          </div>
        ...
        {filteredResults.length === 0 && !loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-center gap-2">
            ...
            {(subDubFilter !== 'all' || minRating > 0 || sortBy !== 'none') && (
              <button
                ...
                className="mt-4 px-4 py-1.5 rounded-xl bg-zinc-900 text-zinc-300 hover:text-white border border-zinc-850 text-xs font-bold transition-all"
              >
                Reset Filters
              </button>
            )}
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {filteredResults.map((anime) => (
                <div
                  ...
                  className="group relative bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800 cursor-pointer hover:border-zinc-700 transition-all active:scale-95 animate-fade-in"
                >
                  <div className="aspect-[3/4] relative overflow-hidden">
                    ...
                    <div className="absolute top-2 right-2 px-2 py-0.5 rounded-md bg-black/60 backdrop-blur-md text-[10px] font-bold text-amber-300 flex items-center gap-0.5">
                      <Star className="w-3 h-3 fill-amber-400" /> {anime.score || '7.5'}
                    </div>
                    {anime.is_dub !== undefined && anime.is_dub > 0 && (
                      <div className="absolute bottom-2 left-2 px-1.5 py-0.5 rounded bg-pink-500 text-[9px] font-black text-white uppercase tracking-wider select-none">
                        DUB
                      </div>
                    )}
                  ...
                  <div className="p-2.5">
                    <h4 className="text-xs font-semibold text-zinc-100 truncate">{anime.title}</h4>
            ...
            {/* Load More Button for Genre pagination */}
            {selectedGenre && hasMore && filteredResults.length > 0 && (
              <div className="flex justify-center mt-6 mb-8">
                <button
                  onClick={handleLoadMore}
                  disabled={loadingMore}
                  className="px-6 py-2.5 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-zinc-200 hover:text-white text-xs font-bold transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50"
                >
                  {loadingMore ? (
                    <>
